...
int by_year_comparator(const void *a, const void *b) {
    ...
    if (entry_a->year > entry_b->year) return 1;
    else if (entry_a->year < entry_b->year) return -1;
    else return 0;
}
...
int by_track_number_comparator(const void *a, const void *b) {
    ...
    if (entry_a->track_num > entry_b->track_num) return 1;
    else if (entry_a->track_num < entry_b->track_num) return -1;
    else return 0;
}
...
void playlist_sort(playlist_t playlist, playlist_sorting_criterion_t order_by) {
    if (playlist == NULL || playlist->entries == NULL || playlist->size == 0) return;
    ...
    switch(order_by) {
        ...
        case ALBUM:
            ...
            break;
        case YEAR:
            upo_merge_sort(playlist->entries, playlist->size, sizeof(entry_t), by_year_comparator);
            break;
        case TRACK_NUMBER:
            upo_merge_sort(playlist->entries, playlist->size, sizeof(entry_t), by_track_number_comparator);
            break;
        case TRACK_TITLE:
            upo_quick_sort(playlist->entries, playlist->size, sizeof(entry_t), by_track_title_comparator);
            break;
        default:
...
int parse_entry(const char *str, entry_t *entry) {
    ...
    /* We need to copy the original string since strtok modifies its first
     * argument */
            ...
            switch (field) {
                case 0: /* artist */
                    ...
                    if (entry->artist == NULL) {
                        upo_throw_sys_error(
                            "Unable to allocate memory for artist name");
                    }
                ...
                case 1: /* album */
                    ...
                    if (entry->album == NULL) {
                        upo_throw_sys_error(
                            "Unable to allocate memory for album name");
                    }
                ...
                case 4: /* track title */
                    ...
                    if (entry->track_title == NULL) {
                        upo_throw_sys_error(
                            "Unable to allocate memory for track title");
                    }
    ...
    if (ok) {
        if (strlen(entry->artist) == 0 || strlen(entry->album) == 0 ||
            entry->year <= 0 || entry->track_num <= 0 ||
...
playlist_t playlist_create_from_file(const char *file_name) {
    ...
    while (upo_io_read_line(fp, &line, &line_len) > 0) {
        ...
        tmp_entries =
            realloc(playlist->entries, (playlist->size + 1) * sizeof(entry_t));
        if (tmp_entries == NULL) {
            ok = 0;
            break;
        }
...
void playlist_print(const playlist_t playlist, FILE *fp) {
    ...
    for (i = 0; i < playlist->size; ++i) {
        fprintf(fp, "%c%s%c%s%c%d%c%d%c%s%c\n", PLAYLIST_ENTRY_DELIMITER,
                playlist->entries[i].artist, PLAYLIST_ENTRY_DELIMITER,
                playlist->entries[i].album, PLAYLIST_ENTRY_DELIMITER,
                playlist->entries[i].year, PLAYLIST_ENTRY_DELIMITER,
                playlist->entries[i].track_num, PLAYLIST_ENTRY_DELIMITER,
                playlist->entries[i].track_title, PLAYLIST_ENTRY_DELIMITER);
    }
}
