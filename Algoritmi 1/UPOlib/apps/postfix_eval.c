    ...
    for (cur = expr; *cur != '\0'; cur++) {
        ...
        if (isdigit(*cur)) {
            ...
            *val = strtol(cur, &endPtr, 10);
            upo_stack_push(stack, val);
        } else if (strchr(operators, *cur) != NULL) {
            long A = *((long *)upo_stack_top(stack));
            upo_stack_pop(stack, 1);

            long B = *((long *)upo_stack_top(stack));
            upo_stack_pop(stack, 1);

            long *calc_res = malloc(sizeof(long));
            *calc_res = exec_op(B, A, *cur);
            upo_stack_push(stack, calc_res);
        } else {
...
void eval_lines(FILE *fp) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        /* Checks if we have the expected result. In this case the input line
         * should be: <expr>, <result> */
        res_str = strstr(line, "=>");
        if (res_str != NULL) {
            line[res_str - line] = '\0';
            res_str += 2;
        ...
        if (ok) {
            if (res_str != NULL) {
                printf(
                    "Expression '%s' -> %ld (expected: %ld -> %s)\n",
                    line, res, expect_res, (res == expect_res) ? "OK" : "KO"
                );
            } else { printf("Expression '%s' -> %ld\n", line, res); }
        } else { printf("Expression '%s' -> Malformed\n", line); }
    }
}
...
void usage(const char *progname) {
    ...
    fprintf(
        stderr,
        "-f <filename>: The full path name to the file containing strings (one "
        "for each line).\n"
...
int main(int argc, char *argv[]) {
    ...
    if (opt_verbose) { printf("-- Evaluating...\n"); }

    eval_lines(fp);

    if (opt_filename != NULL) { fclose(fp); }
